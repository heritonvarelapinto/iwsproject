<?php
		...
		?>				
	        ...
	        <table width="558" cellspacing="1" cellpadding="4" border="0" class="BordaTabela">								
				...
				<tr class="Linha1Tabela">
	                <form action="" name="categoria" method="post" onsubmit="return valida_dropmenu(document.categoria.idcategoria,'categoria');"/>
	                <td height="20" align="right">Pagina:</td>
	                <td >
				        <select class="FORMbox" name="idcategoria">
				            <option value="">--Selecione--</option>
				            <option value="1000">Página Inicial</option>			            
				            <?
							$rs = mysql_query("select * from bairro ORDER BY bairro");
							while($var = mysql_fetch_array($rs)) {
								print('<option value="'.$var["idbairro"].'">'.$var["bairro"].'</option>');
							}
							?>			            
				        </select>
					</td>
			...
			<table width="558" cellspacing="1" cellpadding="4" border="0" class="BordaTabela">
				<tr class="Linha1Tabela">
					<td colspan="3">
						<? //AdmBanners::mostraBannersTopo($idcat); ?>
					</td>
				</tr>
				<tr class="Linha1Tabela">
					<td colspan="3">
						<? //AdmBanners::mostraBannersPeq($idcat); ?>
					</td>
				</tr>
				<tr class="Linha1Tabela">
					<td colspan="3">
						<? //AdmBanners::mostraBannersLat($idcat); ?>
					</td>
				</tr>			
			</table>
			<? } ?>
	<?	}
		...
		function mostraBannersTopo($idcat) { ?>		
			<table width="558" cellspacing="1" cellpadding="4" border="0" class="BordaTabela">
				<tr class="TituloTabela">
					<td  colspan="3" class="label2">Página [<? if($idcat == "1000") { echo "Página Inicial"; }else{ echo classAdmSQL::get("bairro","idbairro",$idcat)->bairro; } ?>]</td>
				</tr>			
				<tr class="TituloTabela">
					<td  colspan="3" class="label2">Banners Grande Topo (468x80)</td>
				</tr>								
				<? $rs = AdmBanners::getBannerRS("topo",$idcat);											
						$i=1;
						...
						?>
							<tr class="Linha1Tabela">
								<td width="25%" align="center">Banner <?=$i;?><br><input type="button" class="bttn4" value="Alterar" onclick="altBanner('<?=$banners->idbanner;?>','468','80');"><br><?AdmBanners::data($banners->idbanner);?></td>							
								<td align="center">
				...
				<? if($idcat == 1000) { ?>
				<tr class="Linha1Tabela">
					<td colspan="2" align="center">Tamanho Máximo<br>480x80<br><br><input type="button" class="bttn2" value="Adicionar Banner" onclick="addBanner('<?=$idcat;?>','topo','<?=$i;?>','468','80')"></td>
				</tr>
				<? }else{ ?>
					<tr class="Linha1Tabela">
						<td colspan="2" align="center">Tamanho Máximo<br>480x80<br><br><input type="button" class="bttn2" value="Adicionar Banner" onclick="addBanner('<?=$idcat;?>','topo','<?=$i;?>','480','80')"></td>
					</tr>
				<? } ?>							
			</table>
	<?	}
