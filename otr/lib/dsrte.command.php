...
abstract class dsRTECommand
{
    ...
    {
        return empty( $this->attributes ) ? '' : '$("#cmd-'.$this->command.'").attr({'.implode( ',', $this->attributes ).'});';
    }
     ...
     */
    protected static function isMSIE()
    {
...
{
    /**
     * Return HTML for either block or newline separator.
     */
...
class dsRTECommandSelect extends dsRTECommand
{
    ...
    protected function getHTML()
    {
        $html .= '<select id="cmd-'.$this->command.'" size="1"><option value="">'.$this->title.'</option>';
...
class dsRTECommandButton extends dsRTECommand
{
    ...
    protected function getHTML()
    {
        $html = '<a class="cmd" id="cmd-'.$this->command.'" title="'.$this->title.'">';
